    ...
    def animate(mode):
        ...
        if mode.player.posX < 100 and mode.frame == 0:
            ...
            if functionCollided == None:
                mode.player.crossingFunction = False
                return
            mode.player.posY += 5*functionCollided.getDerivative(mode.player.posX-1, mode.frame)
            mode.player.posX += 5
...
Click Add Button to create new function""")
    ...
    def saveStats(mode):
        path = "stats.csv"
        fields = [mode.player.currLevel, mode.player.score, mode.player.stars, len(mode.player.myFunctions)]
